...
export async function loadTagOptions(): Promise<TagOption[]> {
  const rows = await db
    .select({ id: tags.id, name: tags.name, label: tags.label, color: tags.color, scope: tags.scope, description: tags.description })
    .from(tags)
    .orderBy(asc(tags.name));
  return rows.map((tag) => ({ id: tag.id, name: tag.label ?? tag.name, color: tag.color, scope: tag.scope as TagScope, description: tag.description }));
}
